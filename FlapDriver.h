...
#ifndef FLAPDRIVER_COMMAND_BUFFER_SIZE
#define FLAPDRIVER_COMMAND_BUFFER_SIZE 512
#endif

class FlapDriver {
public:
	typedef void (*CommandHandler)(FlapDriver* driver, const char* cmd, size_t cmdLen);
...
private:
	char commandBuffer[FLAPDRIVER_COMMAND_BUFFER_SIZE];
...
public:
	FlapDriver(FStepper* steppers, size_t stepperCount, Stream& stream)
		:steppers(steppers), stepperCount(stepperCount), stream(stream)
	{
		commandBufferSize = 0;
		commands = nullptr;
		commandCount = 0;
...
private:
	...
	static void helpHandler(FlapDriver* driver, const char* cmd, size_t cmdLen)
	{
